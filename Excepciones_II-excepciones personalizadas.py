...
def CalcularRaizCuadrada(num):
    if num<0:
        raise ValueError ("El número no puede ser negativo") #Defino un error de ValueError y le asigno un texto para cuando de error
    else:
...
def ValidarNumero(n):
    try:
        ...
        print("El valor introducido no es un número.", error)        
        return False
